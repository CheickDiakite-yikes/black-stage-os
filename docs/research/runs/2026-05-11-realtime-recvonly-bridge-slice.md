...
The first useful event-streaming proofs should be small: one server transcript over the data channel becomes one Stage event, one assistant text event becomes visible Stage speech, and one requested tool call becomes one approval card. That keeps the live path auditable before richer realtime behaviors are turned on.
