...
2. Add Realtime voice contracts in `packages/voice-core`:
   ...
   - Status: implemented as a local-only contract with `gpt-realtime-2` defaults, simulation mode, server-broker policy, WebRTC broker plan, trusted-server unified WebRTC request envelope, Realtime-to-Stage event mapper, safety identifier readiness checks, and Node tests. No actual server route or OpenAI network call runs yet.
